...
- [Teacher submission package](#teacher-submission-package)
...
The following inventory describes the tracked project files included in the repository, excluding installed dependencies, build output, Git metadata, local logs, and deployment metadata.
...
| `server/_core/index.ts` | Express server entrypoint and Vite integration bootstrap. |
...
| `server/_core/sdk.ts` | Server-side session and platform integration helpers. |
...
| `server/_core/types/manusTypes.ts` | Type declarations for the authentication and session integration layer. |
...
Install Node.js 22 or later, pnpm, and R 4.3 or later. For a demonstration-only web deployment, the precomputed analysis artifacts are already committed, so R is only required when rerunning the analysis pipeline. The R packages used by the analysis include `dplyr`, `tidyr`, `ggplot2`, `randomForest`, `rpart`, and `jsonlite`.
...
## Teacher submission package

Submit the repository as a complete analytical product rather than as a collection of screenshots. Use the following links and files:

| Item | Submission value |
|---|---|
| Source repository | https://github.com/ssophavirak-ai/loan-default-risk-intelligence |
| Live demonstration | https://loan-default-risk-intelligence.onrender.com/ |
| Formal report | [`docs/PROJECT_REPORT.md`](docs/PROJECT_REPORT.md) |
| Demonstration script | [`docs/VIDEO_SCRIPT.md`](docs/VIDEO_SCRIPT.md) |
| Reproducible analysis | [`analysis/run_analysis.R`](analysis/run_analysis.R) |

Before submitting, open the live link in a private browser window, confirm that the dashboard loads, and run `pnpm check`, `pnpm build`, and `pnpm test` locally. In the written submission, explain the business question, dataset provenance, preprocessing, model comparison, evaluation metrics, dashboard design, limitations, and responsible-use decision. During the demonstration, show the Overview, Model lab, Explainability, Application scorer, and Methodology views in that order.

Use the report as the main written deliverable and the live URL as the primary demonstration. If your instructor requires a compressed source archive, create it from the repository root while excluding `node_modules`, `dist`, `.git`, and local environment files. Follow your institution's rules for documenting external tools or assistance, and be prepared to explain the analysis and implementation choices in your own words.

### Suggested submission message

> Subject: Final Full-Stack Data Analytics Project — Loan Default Risk Intelligence
>
> Dear [Instructor name],
>
> Please find my final project, **Loan Default Risk Intelligence**, a full-stack analytics and visualization application built around the UCI Statlog German Credit dataset. The project includes a reproducible R analysis pipeline, model comparison, cost-aware evaluation, explainability, an interactive review-oriented scorer, automated tests, and a formal report.
>
> GitHub repository: https://github.com/ssophavirak-ai/loan-default-risk-intelligence
> Live demonstration: https://loan-default-risk-intelligence.onrender.com/
>
> The project is framed as decision support for human review and not as an automated credit-approval system. The formal report and reproducibility instructions are included in the repository.
>
> Kind regards,
> Sophavirak Srun
